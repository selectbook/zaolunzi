...
import cn.zaolunzi.model.ViewObject;
...
public class MessageController {
    ...
    public String addMessage(@RequestParam("toName") String toName,
        ...
        try {
            if (hostHolder.getUser() == null) {
                return ChaofenUtil.getJSONString(999, "未登录");
            }

            User user = userService.selectByName(toName);
            if (user == null) {
                return ChaofenUtil.getJSONString(1, "用户不存在");
            }
            ...
            return ChaofenUtil.getJSONString(0);

        } catch (Exception e) {
            logger.error("发送消息失败" + e.getMessage());
            return ChaofenUtil.getJSONString(1, "发信失败");
        }
    }
}
